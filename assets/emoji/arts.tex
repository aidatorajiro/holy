...
\newcounter{currentArts}
\setcounter{currentArts}{0}

\newcommand{\showLastArt}{\csname artsDef\roman{maxArts}\endcsname}

\newcommand{\showAllArts}{{
\loop
\newpage
\stepcounter{currentArts}
\csname artsDef\roman{currentArts}\endcsname
\ifnum \value{currentArts}<\value{maxArts}
\repeat
}}
...
% Arguments: randomseed, points, randoffset, randcoeff
\newcommand{\generateCircleWave}[4]{{
\directlua{
    math.randomseed(#1)
    points = #2
    x = nil
    ...
    for i = 0, points do
        ...
        if i == 0 then
            ...
            control_c = math.cos(thbase - math.pi / points)
            control_s = math.sin(thbase - math.pi / points)
            control_x = prev_x + (x - prev_x) / 2
            control_y = prev_y + (y - prev_y) / 2
            control_x = control_x + control_c * (#4) * r
            control_y = control_y + control_s * (#4) * r
        end
...
\begin{center}
\begin{tikzpicture}
\draw[line width=0.2] [line join = round][line cap = round] \generateCircleWave{1}{13}{-0.1}{1};
\draw[line width=0.4] [line join = round][line cap = round] \generateCircleWave{2}{13}{-0.1}{1};
\draw[line width=0.6] [line join = round][line cap = round] \generateCircleWave{3}{13}{-0.1}{1};
\node[scale=4] at (0, 0) {$\mathbf{K}$};
\end{tikzpicture}
\end{center}
}}
...
\begin{center}
\begin{tikzpicture}
\draw[line width=0.2] [line join = round][line cap = round] \generateCircleWave{4}{13}{-0.1}{1};
\draw[line width=0.4] [line join = round][line cap = round] \generateCircleWave{5}{13}{-0.1}{1};
\draw[line width=0.6] [line join = round][line cap = round] \generateCircleWave{6}{13}{-0.1}{1};
\node[scale=4] at (0, 0) {$\mathbf{C}$};
\end{tikzpicture}
\end{center}
}}
...
\begin{center}
\begin{tikzpicture}
\draw[line width=0.2] [line join = round][line cap = round] \generateCircleWave{4}{26}{0}{0.3};
\draw[line width=0.4] [line join = round][line cap = round] \generateCircleWave{5}{13}{0}{0.6};
\draw[line width=0.6] [line join = round][line cap = round] \generateCircleWave{6}{7}{0}{0.6};
\node[scale=0.3] at (-0.5, 0.3) {$\Lambda$};
\node[scale=0.3] at (0.5, 0.3) {h};
\node[scale=1] at (0, -0.35) {$\mathfrak{O}$};
\end{tikzpicture}
\end{center}
}}